...
class Pelicula extends Model {
  ...
  public function save() {
    ...
    if (!isset($this->id)) {
      throw new Exception("¿Estas obteniendo una película mediante find?");
    }
    if ($this->id !== $request->secret) {
      throw new Exception("La película que esta siendo almacenada no tiene el id que llega en la URL");
    }
    
    if (!isset($this->title) || $this->title !== $request->title) {
      throw new Exception("La película que esta siendo almacenada no tiene el título dado en el request");
    }
    
    if (!isset($this->rating) || $this->rating !== $request->rating) {
      throw new Exception("La película que esta siendo almacenada no tiene el rating dado en el request");
    }
    
    if (!isset($this->awards) || $this->awards !== $request->awards) {
